...
riser_h      = 13;  // mm, straight vertical shaft above the ogee
...
assert(platform_h > 0,  "height too small: no room for the platform");

// 3. Profile (list of [half-width, z], bottom -> top)
//    Ogee: full-period cosine S — convex lower half, concave upper half,
//    vertical tangents at both ends (blends into the flat base and riser).
ogee_pts = [ for (i = [0 : arc_steps])
    let (t = i / arc_steps)
    [ hw1 + ogee_inset * (1 + cos(180 * t)) / 2, ogee_h * t ] ];
...
profile = concat(
    ogee_pts,                       // base S-curve, ends at [hw1, z_riser]
